// 📍 Admin Add Road Page - addroad.tsx (Restored marker style with intersection support)
...
export default function AddRoad() {
  ...
  const handleMapPress = (event: MapPressEvent) => {
    ...
    const snappedIntersection = intersections.find(i => {
      const dist = Math.sqrt(Math.pow(i.lat - latitude, 2) + Math.pow(i.lng - longitude, 2));
      return dist < 0.0005;
    });

    const newPoint = snappedIntersection
      ? { lat: snappedIntersection.lat, lng: snappedIntersection.lng, isIntersection: true }
      : { lat: latitude, lng: longitude };

    if (mode === 'start') {
      setStartPoint({ lat: newPoint.lat, lng: newPoint.lng });
    } else if (mode === 'middle') {
      setMiddlePoints(prev => [...prev, newPoint]);
    } else if (mode === 'intersection') {
      if (snappedIntersection) {
        setMiddlePoints(prev => [...prev, { lat: newPoint.lat, lng: newPoint.lng, isIntersection: true }]);
      } else {
        Alert.alert('Invalid', 'Please click on a registered intersection');
      }
    } else if (mode === 'end') {
      setEndPoint({ lat: newPoint.lat, lng: newPoint.lng });
    } else if (mode === 'delete') {
      const THRESHOLD = 0.0005;
      setMiddlePoints(prev => prev.filter(p => {
    ...
    <View style={styles.container}>
      ...
      >
        {savedRoads.map((road, idx) => (
          <Polyline
            key={idx}
            coordinates={road.path.map((p: any) => ({ latitude: p.lat, longitude: p.lng }))}
            strokeColor="#5089A3"
            strokeWidth={4}
          />
        ))}
        ...
        {intersections.map((intersection, index) => (
          <Marker
            ...
            pinColor="orange"
            title={`${intersection.roadNames.join(' x ')}`}
          />
        ))}
        ...
        {middlePoints.map((mp, i) => (
          <Marker
            ...
            title={mp.isIntersection ? 'Intersection' : `Middle ${i + 1}`}
          />
        ))}
        {endPoint && (

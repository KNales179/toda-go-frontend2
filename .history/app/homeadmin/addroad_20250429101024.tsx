...
import MapView, { Marker, Polyline, MapPressEvent, Callout } from 'react-native-maps';
...
export default function AddRoad() {
  ...
  const findNearestIntersection = (lat: number, lng: number) => {
    return intersections.find(intersection => {
      const dist = Math.sqrt(Math.pow(intersection.lat - lat, 2) + Math.pow(intersection.lng - lng, 2));
      return dist < 0.0005; // ~50 meters
    });
  };
  ...
  const handleMapPress = (event: MapPressEvent) => {
    ...
    if (mode === 'start') {
      const nearest = findNearestIntersection(latitude, longitude);
      if (nearest) {
        setStartPoint({ lat: nearest.lat, lng: nearest.lng });
      } else {
      ...
      if (nearest) {
        setMiddlePoints(prev => [
          ...prev,
          { lat: nearest.lat, lng: nearest.lng, isIntersection: true },
        ]);
      } else {
      ...
      const nearest = findNearestIntersection(latitude, longitude);
      if (nearest) {
        setEndPoint({ lat: nearest.lat, lng: nearest.lng });
      } else {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {/* Small circles for intersections */}
        {intersections.map((intersection, index) => (
          ...
          >
            ...
            <Callout>
              <Text>{intersection.roadNames[0]} x {intersection.roadNames[1]}</Text>
            </Callout>
          </Marker>
        ))}

...
export default function PHome() {
  ...
  useEffect(() => {
    ...
    const pollForBookingCompletion = async () => {
      ...
      try {
        ...
        setAlertedBookingComplete(false);
        setTripCompleted(false);
        console.log(alertedBookingComplete, myBooking.status, tripCompleted, matchedDriver?.driverId)
        const driverIdToRate = matchedDriver?.driverId;
        await AsyncStorage.setItem("driverIdToRate", driverIdToRate || "");
  ...
  const submitDriverRating = async () => {
    try {
      console.log(matchedDriver?.driverId,)
      const storedDriverId = await AsyncStorage.getItem("driverIdToRate");
      if (!storedDriverId) {
        Alert.alert("Error", "Driver ID missing. Please try again.");
        return;
      }

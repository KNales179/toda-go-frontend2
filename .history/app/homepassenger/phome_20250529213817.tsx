  ...
  useEffect(() => {
    ...
    const pollForBookingCompletion = async () => {
      ...
      try {
        ...
        if (myBooking && myBooking.status === "completed") {
          Alert.alert("🎉 Booking Complete", "Thank you for riding with us!");
          // ✅ Reset passenger state here if needed!
        }
      } catch (err) {
                    ...
                    <>
                      <View style={{ flexDirection: "row", alignItems: "center" }}>
                        {matchedDriver.selfieImage && (
                          <Image
                            source={
                              matchedDriver.selfieImage ? { uri: matchedDriver.selfieImage.uri } : matchedDriver?.selfieImage
                                ? { uri: `${API_BASE_URL}/${matchedDriver.selfieImage.replace(/\\/g, "/")}` }
